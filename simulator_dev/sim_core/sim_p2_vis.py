...
#
import sim_p3_interaction_vis as p3
#
...
# DEFAULT_IMPORT_DATAPATH = os.path.join(ROOT, "dump/datasource/dump/OxHum_Interactions/datasource/raw/March2024")
...
def import_data():
    ...
    if selected_filename is not None:
        ...
        gateway_df = gateway_df.sort_values(by=['time'])
        ...
        beacon_df = beacon_df.sort_values(by=['time'])
        data_df_map = {'gateway': gateway_df, 'beacon': beacon_df}
        st.write('Gateway: ', gateway_df.time.min())
        st.write('Beacon: ', beacon_df.time.min())
        return data_df_map
    else :
        return None
...
session_action_list = ['Current', 'Predict Loc: New/Reset', 'Predict Loc: Load Prior Session', 'Diagnostics Loc: Import Data', 'Interactions']
location_tab_options = ["Build Sims", "Register", "Import Gateway Data"]
interaction_tab_options = [ "Interaction Analysis"]
tab_types = ["location", "interaction"]
st.session_state["tab_options"] = {'location': location_tab_options + interaction_tab_options,
                                   'interaction': location_tab_options + interaction_tab_options,
                                   'all': location_tab_options + interaction_tab_options
                                   }
...
with st.sidebar:
    ...
    with col2:
        if st.session_state["session_action"] == session_action_list[0]:
            st.session_state["tab_type"] = 'all'
        if st.session_state["session_action"] == session_action_list[1]:
            st.session_state["tab_type"] = 'location'
            if st.button('create'):
        ...
        if st.session_state["session_action"] == session_action_list[2]:
            ...
            if st.button('load'):
                if loaded_state is not None:
                    ...
                    st.session_state["imported_data"] = {}
                    st.session_state["imported_beacon_data"] = {}
        if st.session_state["session_action"] in session_action_list[3:5]:
            st.session_state["tab_type"] = 'location'
            st.session_state['import_beacon_prefix'] = 'BC' + 'test'.encode('utf-8').hex().upper()
            st.session_state['osm_data'] = {}
        ...
        # Interactions
        if st.session_state["session_action"] == session_action_list[4]:
            st.session_state["tab_type"] = 'interaction'
            st.session_state['interaction_mapdraw_counter'] = 0
            st.radio("Analyse", ['Predict', 'Diagnose'], index=1, key="interaction_analyser")
            if 'imported_data' in st.session_state and (len(st.session_state['imported_data']) > 0):
                if st.button('Setup sims'):
                    sim_interact = p3.sim_interaction_analysis(st.session_state)
                    st.session_state['sim_interaction'] = sim_interact
                    st.session_state['conversion_state'] = False
                    st.session_state['interact_diagnose_valid'] = False
    ...
    if st.session_state['osm_map_load']:
        import_osm_file()
    if st.session_state["session_action"] in session_action_list[:4]:
        st.text_input('Gateway Antenna Height',value=1.5, key='override_gateway_height_m')
    if ("site" in st.session_state) and (st.session_state["session_action"] not in session_action_list[3:]):
        #
...
#
tab_type = st.session_state['tab_type']
tab_options = st.session_state['tab_options'][tab_type]
# if tab_type == 'all':
#     tab_build, tab_reg, tab_import, tab_interact = st.tabs(tab_options)
# elif tab_type == 'location':
#     tab_build, tab_reg, tab_import = st.tabs(tab_options)
# elif tab_type == 'interaction':
#     tab_interact = st.tabs(tab_options)
#
tab_build, tab_reg, tab_import, tab_interact = st.tabs(tab_options)

# if tab_type in ['all', 'location']:
#
with tab_build:
    if st.session_state['tab_type'] != "location":
        st.write('Tab only valid for Locations currently')
    else:
        with st.container():
        ...
        if st.session_state["session_action"] == session_action_list[3]:
            ...
            if 'imported_data' in st.session_state and (len(st.session_state['imported_data']) > 0):
                ...
                with import_col1:
                    ...
                    #
                with draw_col1:
                    st.select_slider('Zoom Levels', options=[i for i in range(10,20)], value=17, key='zoom_level_import')
            ...
            msg = 'Tab only valid for ', session_action_list[3] 
            st.write(msg)

# -- Interact --
# if tab_type in ['all', 'interaction']:
#
with tab_interact:
    with st.container():
        # settings
        if st.session_state["session_action"] == session_action_list[4]:
            if (('imported_data' in st.session_state) and
                (len(st.session_state['imported_data']) > 0) and
                ('sim_interaction' in st.session_state)
                ):
                if st.session_state['interaction_analyser'] in ['Diagnose']:
                    sim_interact = st.session_state['sim_interaction']
                    st.session_state['conversion_key'] = sim_interact.session_state['conversion_key']
                    sim_interact.setup_ui()
                    sim_interact.ui_gateway()
                    sim_interact.define_session_info()
                    if st.button('Form Complete'):
                        sim_interact.find_locations()
                        converted = sim_interact.apply_conversions()
                        st.session_state['conversion_state'] = converted
                        st.session_state['conversion_key'] = sim_interact.session_state['conversion_key']
                    #
                    if st.session_state['conversion_state'] & st.checkbox('Run Conversion Checks'):
                        conv_check_col1, conv_check_col2 = st.columns(2)
                        with conv_check_col1:
                            conv_key = st.selectbox('Check Conversion', st.session_state['conversion_key'], index=0 )
                            val = st.session_state['conversion_key'][conv_key]
                        with conv_check_col2:
                            st.write(val, type(val))
                    if st.button('Run interaction diagnostics'):
                        interact_diagnose_valid = sim_interact.run_diagnostics()
                        st.session_state['interaction_mapdraw_counter'] = 0
                        st.session_state['interact_diagnose_valid'] = interact_diagnose_valid
                        msg = f'Interaction Valid: {interact_diagnose_valid}'                        
                        st.write(msg)
                    if st.session_state['interact_diagnose_valid']:
                        #sim_interact.setup_vis()
                        draw_col1, draw_col2 = st.columns(2)
                        with draw_col1:
                            dc = st.session_state['interaction_mapdraw_counter']
                            dc = sim_interact.show_gps_map(dc)
                            # if dc == 0:
                            #     dc = sim_interact.show_data_map(dc)
                            #     st.session_state['interaction_mapdraw_counter'] = 0
                        with draw_col2:
                            sim_interact.show_rssi_plots()

        else:
            st.write('Tab only valid for Interactions')
